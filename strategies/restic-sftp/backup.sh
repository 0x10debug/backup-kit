#!/usr/bin/env bash
# strategies/restic-sftp/backup.sh — Run a Restic backup over SFTP
...
# SFTP password is optional (key-based auth is preferred). If set, export it
# so sshpass can use it when Restic invokes sftp.
if [ -n "${SFTP_PASSWORD:-}" ]; then
    export SFTP_PASSWORD
    # Restic reads SFTP_PASSWORD directly for sftp backend auth.
fi
